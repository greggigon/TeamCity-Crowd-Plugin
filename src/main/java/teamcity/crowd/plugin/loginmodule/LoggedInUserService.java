...
public class LoggedInUserService {
    private final GroupNameToGroupKey groupNameToGroupKey = new GroupNameToGroupKey();
    ...
    public LoggedInUserService(UserGroupManager userGroupManager, PluginCrowdClient pluginCrowdClient, UserModel userModel, LoggerFactory loggerFactory, CrowdPluginConfiguration crowdPluginConfiguration) {
    ...
    private SUser updateUserDetails(SUser teamCityUser, User user) {
        ...
        if (!associatedUser.getName().equals(user.getDisplayName()) || !associatedUser.getEmail().equals(user.getEmailAddress())) {
    ...
    private void updateUserGroups(SUser teamCityUser) {
        ...
        final List<String> listOfGropusUserShouldBeRemovedFrom = groupsToBeRemovedFrom(allTeamCityGroupsUserIsAMemberOfAlready, userGroupsInCrowd);
        ...
        for (String userGroup : userGroupsInCrowd) {
            if (!allTeamCityGroupsUserIsAMemberOfAlready.contains(userGroup)) {
                ...
                if (teamCityGroup == null && shouldCreateGroups) {
                    teamCityGroup = userGroupManager.createUserGroup(groupNameToGroupKey.transform(userGroup), userGroup, CREATED_BY_PLUGIN_MESSAGE);
                }
    ...
    private List<String> userGroupsInCrowd(SUser teamCityUser) {
        return Lists.newArrayList(Collections2.transform(pluginCrowdClient.getUserGroups(teamCityUser.getUsername()), new Function<Group, String>() {
            @Override
    ...
    private List<String> allTeamCityUserGroups(SUser teamCityUser) {
        return Lists.newArrayList(Collections2.transform(teamCityUser.getUserGroups(), new Function<UserGroup, String>() {
            @Override
